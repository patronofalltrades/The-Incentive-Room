...
export const SPECIAL_COMPONENTS: Simulation = {
  ...
  steps: [
    // ===== Part A: Q1 =====
    {
      ...
      approach: [
        ' Identify relevant costs from the company-wide perspective. The transfer price is an internal accounting entry and cancels out. Only real cash flows matter: revenue from Newton, actual production costs, shipping, additional labor, and financing.',
        ' Compute incremental revenue. Newton pays $100/unit \u00D7 10,000 = $1,000,000.',
        ' Compute incremental costs. Variable manufacturing at WCO: $33 \u00D7 10,000 = $330,000. Shipping WCO to ECO: $3,000. Additional labor at WCO: 4 workers \u00D7 $30,000 = $120,000. Financing of NFO: 8% \u00D7 $100,000 = $8,000. Fixed manufacturing costs do not change (WCO has idle capacity). Corporate OH does not change in total ($1,200,000 either way).',
        ' Compute net effect. \u0394 profit = $1,000,000 \u2212 $330,000 \u2212 $3,000 \u2212 $120,000 \u2212 $8,000 = +$539,000. The order is clearly beneficial for shareholders because WCO\'s idle capacity means the incremental cost is just the variable cost plus the small additions.',
      ],
      answer:
    ...
    // ===== Part A: Q2 =====
    {
      ...
      approach: [
        ' Compute the transfer price. TP = full standard cost + 50% markup = $63 \u00D7 1.5 = $94.50/unit. WCO\'s revenue from the transfer = $94.50 \u00D7 10,000 = $945,000.',
        ...
        ' Compute WCO\'s divisional income change. Revenue from TP: +$945k. Variable cost: \u2212$330k. Shipping: \u2212$3k. Labor: \u2212$120k. Financing: \u2212$8k. OH reallocation: \u2212$12k. Net = +$472k.',
        ' Compute ECO\'s divisional income change. Revenue from Newton: +$1,000k. TP payment to WCO: \u2212$945k. OH reallocation: +$12k (receives less OH). Net = +$67k.',
        ' Both divisions benefit. WCO: +$472k, ECO: +$67k. Note that $472k + $67k = $539k = the company-level gain, confirming consistency.',
      ],
      answer:
    ...
    // ===== Part A: Q3 =====
    {
      ...
      approach: [
        ' Define goal congruence. Goal congruence means that when each division manager acts in their own interest (maximizing their divisional income), the outcome is also optimal for the company as a whole.',
        ' Check alignment. SC (company) benefits: +$539k. WCO benefits: +$472k. ECO benefits: +$67k. Both divisions want to accept, and the company wants to accept. The decisions are aligned.',
        ' Conclude with a caveat. In this specific case, there is goal congruence. However, goal congruence in one scenario does not guarantee it in all scenarios. The TP policy might fail in other situations (as Part B will demonstrate).',
      ],
      answer:
    ...
    // ===== Part B: Q4 =====
    {
      ...
      approach: [
        ' Identify costs that differ between alternatives. Revenue ($1M) and variable manufacturing cost ($330k) are the same either way. The differences are: shipping, additional labor, and machine rental.',
        ' Compare incremental costs. At WCO: shipping $3k + labor 4 \u00D7 $30k = $120k + financing $8k = $131k total. At ECO: rent $16k + labor 8 \u00D7 $30k = $240k + financing $8k = $264k total. No shipping needed since ECO produces and sells directly.',
        ' Compute the difference. WCO is cheaper by $264k \u2212 $131k = $133k. The key drivers: WCO needs only 4 additional workers (using existing idle equipment) vs. ECO\'s 8 workers plus machine rental.',
        ' Conclude. SC should prefer production at WCO. Utilizing existing idle capacity is more cost-effective than expanding capacity at a fully utilized facility.',
      ],
      answer:
    ...
    // ===== Part B: Q5 =====
    {
      ...
      approach: [
        ' Compute ECO\'s divisional income if self-produced. Revenue = $1,000k. Variable cost = $330k. Labor = 8 \u00D7 $30k = $240k. Financing = $8k. Rent = $16k. OH reallocation: ECO labor goes from $3M to $3.24M, rate = $1.2M / $6.24M = 0.192. ECO OH = 0.192 \u00D7 $3.24M = $623k (up $23k from $600k baseline). Net = $1,000k \u2212 $330k \u2212 $240k \u2212 $8k \u2212 $16k \u2212 $23k = +$383k.',
        ' Compare with sourcing from WCO. From Part A Q2, ECO\'s income if sourcing from WCO = +$67k.',
        ' Identify Cathy\'s preference. $383k >> $67k. Cathy strongly prefers self-production even though WCO production is $133k better for the company. The TP policy ($94.50/unit) transfers most of the value to WCO, leaving ECO with only $67k if it sources internally.',
        ' Diagnose the problem. The high transfer price ($94.50 = 150% of full cost) means ECO captures very little of the order\'s value when sourcing from WCO. Self-production lets ECO keep the full margin minus its own costs.',
      ],
      answer:
    ...
    // ===== Part B: Q6 =====
    {
      ...
      approach: [
        ' Identify the goal congruence failure. Part A showed goal congruence: both divisions and the company wanted to accept via WCO. But Part B reveals that when ECO has the option to self-produce, Cathy prefers self-production (+$383k divisional income) over sourcing from WCO (+$67k), even though WCO production is $133k cheaper for the company.',
        ' Diagnose the root cause. The transfer price at 150% of full cost is too high. It shifts almost all the value of the internal transfer to WCO, leaving ECO with minimal margin. This gives ECO a strong incentive to bypass the transfer and self-produce, even at higher company-wide cost.',
        ' Note the broader implication. The same TP policy that achieved goal congruence in one scenario (Part A) fails in another (Part B). This demonstrates that goal congruence must be evaluated across all possible decision scenarios, not just one.',
      ],
      answer:
    ...
    // ===== Part B: Q7 =====
    {
      ...
      approach: [
        ' Adjust the transfer price. Reduce the markup so ECO captures more of the value from internal transfers. For example, a lower markup or a negotiated price between $63 (full cost) and $94.50 would give ECO a larger share of the $539k gain, reducing the incentive for costly self-production.',
        ' Change the performance measure. Measure divisional performance independent of the transfer price \u2014 for instance, using contribution margin above variable cost. This sidesteps the transfer pricing distortion entirely.',
        ' Align compensation with corporate goals. Tie a portion of each manager\'s bonus to overall corporate profits, not just divisional income. This gives both managers an incentive to consider the company-wide impact of their decisions.',
        ' Fix the overhead allocation. Allocate corporate overhead based on budgeted costs or capacity rather than actual costs. This prevents the OH reallocation effect from distorting divisional decisions.',
      ],
      answer:
